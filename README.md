...
- **Streaming live analyses and follow-up chat** — typed or edited flows now stream friction points one at a time instead of returning as a single 25-second blob. First card lands in ~2–4 s. The user can start reading and arguing with card #1 while cards #2–5 are still being generated. A pulsing "N ready so far" indicator sits below the last card until the stream completes and the summary block lands at the end. Follow-up chat answers stream progressively the same way — once a user has seen the main analysis appear word by word, they expect the chat to behave the same. Built on Anthropic's SSE API plus `partial-json` for tolerant mid-stream parsing.

- **Prompt caching on the system prompts** — the ~2.5k-token analyzer prompt and the per-session chat prompt (which carries the full analysis JSON) are both marked with `cache_control: ephemeral`. Repeat analyses and multi-question chat sessions inside a 5-minute window hit Anthropic's 90% cached-token discount and save ~1 s on first byte. Invisible cost and perf win — no user-facing change, just a platform-aware optimization that makes sense when the request is identical across calls except for the last user message.
