...
export function exportToPDF(result: AnalysisResult, flowDescription: string): void {
  ...
  console.log('[Activation Lab] PDF generated, calling doc.save()')
  try {
    doc.save('activation-lab-brief.pdf')
    console.log('[Activation Lab] doc.save() returned without throwing')
  } catch (saveErr) {
    console.error('[Activation Lab] doc.save() threw:', saveErr)
    throw saveErr
  }
}
